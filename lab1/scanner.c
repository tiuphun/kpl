...
Token* getToken(void) {
	...
	case 2:
		...
		if (charCodes[currentChar] == CHAR_SPACE) state = 2;
		else state = 0;
		return getToken();
	case 3:
	...
	case 24: 
		readChar();
		if (charCodes[currentChar] == CHAR_RPAR) state = 25; else state = 26;
		return getToken();
	case 25:
		readChar();
	...
	case 28:
		readChar();
		if (charCodes[currentChar] == CHAR_EQ) state = 29; else state = 30;
		return getToken();
	case 29:
		readChar();
	...
	case 31:
		...
		else if (isprint(currentChar)) state = 32;
			else state = 34;
		return getToken();
	case 32:
		c = currentChar;
	...
	case 35: // tokens begin with lpar, skip comments
		...
		if (currentChar == EOF)
			state = 41;
		else switch (charCodes[currentChar]) {
			case CHAR_PERIOD:
				state = 36;
				break;
			case CHAR_TIMES:
	...
	case 37:
		...
		if (currentChar == EOF) {
			...
			readChar(); // one more asterisk
			if (charCodes[currentChar] == CHAR_TIMES) state = 38;
		}
		else state = 37;
		return getToken();
	case 38:
