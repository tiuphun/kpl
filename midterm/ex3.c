...
Token* getToken(void)
{
	...
	case 12:
        ...
        if (currentChar != EOF && charCodes[currentChar] == CHAR_SLASH) {
            ...
            while (currentChar != EOF && charCodes[currentChar] != '\0') {
                readChar();
            }
    ...
    if (currentChar == EOF)
        ...
        case CHAR_PERIOD:
            state = 36;
            break;
        case CHAR_SLASH: // changed from CHAR_TIMES
            state = 37; // changed from 38
            break;
        default:state = 41;
        }
    return getToken();
    ...
    case 37:
        while ((currentChar != EOF) && (charCodes[currentChar] != CHAR_NL)) // changed from CHAR_TIMES to CHAR_NL
        {
            state = 37;
            readChar();
        }
        if (currentChar == EOF)
            state = 40;
        else state = 0; // changed from 38 to 0
        return getToken();
    // removed cases 38, 39 as they are not needed for single line comments
    case 40:
