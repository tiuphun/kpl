...
Token* getToken(void)
{
	...
	case 13:
		...
		else if (charCodes[currentChar] == CHAR_GT) state = 21; // SB_NEQ is <>
		else state = 15;
		return getToken();
	case 14:
		readChar();
	...
	case 32:
		...
		if (charCodes[currentChar] == CHAR_SINGLEQUOTE) {
			str[count] = '\0';
			state = 33;
		} else if (isprint(currentChar) && count < 255) {
			str[count++] = currentChar;
		} else
			state = 34;
		break;
	case 33:
		if (count == 1) {
			token = makeToken(TK_CHAR, lineNo, colNo - count);
		}
		else {
			token = makeToken(TK_STRING, lineNo, colNo - count);
		}
		strcpy(token->string, str);
		readChar();
		return token;
	case 34:
		error(ERR_INVALIDSTRINGCONSTANT, lineNo, colNo - count);
		return makeToken(TK_NONE, lineNo, colNo - count);
	case 35:
		ln = lineNo;
		cn = colNo;
		readChar();
